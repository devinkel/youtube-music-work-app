...
<main class="container">
    ...
    <div v-else class="card">
        ...
        type="text"
        placeholder="🔍 Qual é a musica Lombardi?"
        class="text-field"
        />
        <button
        @click="searchSongs"
...
export default {
    ...
    methods: {
        ...
        async searchSongs() {
            ...
            this.results = []
            try {
                const res = await fetch(`${import.meta.env.VITE_API_URL}/preview?query=${encodeURIComponent(this.input.trim())}`)
        ...
        async addSong(videoId) {
            ...
            this.message = ''
            try {
                const res = await fetch(import.meta.env.VITE_API_URL + '/add-song', {
                    method: 'POST',
                ...
                this.isError = false
                this.message = `Musica adicionada`
                // Marca como duplicada na UI
                this.results = this.results.map(r =>
                    r.videoId === videoId ? { ...r, duplicate: true } : r
                )
            } catch (err) {
                this.isError = true
                this.message = err.message
